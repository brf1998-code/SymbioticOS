...
| S1-09 | On the live instance with Fable: ship a small UI change for a named person, answer Not quite with a specific gap, review the follow-up | Read the follow-up's proposal against the first one: it names the gap, keeps what worked, and the build touches only what is still off |
| S1-08 | On the live instance, with a real ERP lookup published: file feedback asking for one more ERP field on a screen, approve, confirm, build, deploy | The proposal names the published field; the build adds it to the lookup's `fields` in module.json and shows it; the run log says every ERP field is available; nobody touched the connections page |
...
2. **No automated backup.** Snapshots live in the same database they protect.
   S3-17.
3. **`smokeCheck` only fails on 500 and above** (`src/pipeline.js`). A staged
   page returning 404 passes both the visual check and the test run. S3-06.
