...
Status 2026-09-19: the files, printer and ERP (read only, named queries; SAP, Epicor and plain JSON flavors) kinds are built as designed here (`src/connections.js`, the connections page, `ctx.connections`, the print helper; CLAUDE.md has the detail). The generated walkthrough and the bridge transport are not; the ERP kind carries a `bridge` transport setting that answers "not available yet". Decisions taken: SAP sets the ERP pattern; the manager sets up printers and spreadsheets, the admin enters ERP logins during pilots; NEWP's SAP reach is unknown, so the ERP kind must run a named query either from the instance or through the bridge.
